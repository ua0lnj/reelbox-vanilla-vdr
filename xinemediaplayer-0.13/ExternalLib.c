...
bool ExternalLib::Play(const char* mrl) {
	...
	if(0 == pid) {
		...
		int hdmi_opt = -1;
		int spdif_opt = -1;
		cSetupLine *line = GetSetupLine("hdmi_audio", "reelice");
		if(line) hdmi_opt = atoi(line->Value());
		line = GetSetupLine("spdif_audio", "reelice");
...
cSetupLine *ExternalLib::GetSetupLine(const char *Name, const char *Plugin)
{
