...
try {
        ...
        else if ( cXineOsd::k_blue == WEBVIDEO_PLUGIN)
            cRemote::CallPlugin("webvideo");
#endif
    ...
    void Player::GstProcessKey(eKeys key) {
        ...
        double playback_rate = gstLib_.PlaybackRate();
            ...
            case kFastRew:
                ...
                if (!HasVideo())
                {
                    esyslog("Has no video. No FF or Frew. Playback rate %f\n", playback_rate);
                    break;
                }
