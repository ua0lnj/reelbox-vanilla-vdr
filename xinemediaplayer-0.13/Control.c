...
{ 
    ...
    /* xinemedia player can run without opening the xineOsd, instead it could open
     * any other plugin's menu. Example: music browser or youtube (?)
     */
    void OpenMenu()
    {
        // was no OSD requested ?
        if (cXineOsd::browserInstance <= -5)
        {
            // try to call the external plugin
            if (cXineOsd::CallExt_Plugin())
                return ;
            else
            {
                isyslog("no external plugin opened for k_blue: %d", cXineOsd::k_blue);
            }
        } // no xineOsd

        // fallback or requested xineOsd
...
#if 0 // handle Ok-key and bring up menu in ProcessKey2()
      // get OSD again
        ...
        switch (key) {
          case kBack: 
            if (visible_) {
                ...
                OpenMenu();
                printf("\033[7;91m Got kBack and no visible osd. Showing Playlist menu.\033[0m\n");
            }
            return osContinue;
          case kBlue:
          case kStop: 
            Hide();
            player_->Activate(false);
            printf("\033[7;91m Got kStop. Returning osEnd\033[0m\n");
            return osEnd;
          case kOk:
            if (visible_ && !modeOnly_) {
                ...
                // In dvd menu domain do not open replay osd
                if (!player_->IsReplayingDvd())
                    Show();

                /* No osd was present and okay key was pressed, make sure some osd is shown
                   if no osd is open; in the case of shoutcast streams with Total=0
                   at least open the playlist menu.
                   If inside DVD menu, then no osd should be shown.
                 */
                if (!cOsd::IsOpen() && !(player_->IsReplayingDvd() && player_->PlayerIsInMenuDomain()))
                    OpenMenu();
            } // if#
            return osContinue;
          default: 
      ...
      if (player_  && !(player_->IsReplayingDvd() && player_->PlayerIsInMenuDomain())) { 
        ...
        switch (key) {
          ...
          default: {
            ...
            // donot not return osUnknown from cControl::ProcessKey() as 'state' is osContinue
            // colorkeys are overridden in vdr-1.7
            if (state1 != osUnknown)
                state = state1;
   ...
   void Control::ShowMode()
   {
       if (Setup.ShowReplayMode)
       {
           ...
           if (GetReplayMode(Play, Forward, Speed))
           {
               ...
               if (modeOnly_ && !timeoutShow_ && (Play == 1 && Speed == -1))
                  timeoutShow_ = time(NULL) + MODETIMEOUT; /// set timeout; keeps icon open if not normal play
...
//            SetHelp(trVDR("Jump"), trVDR("Back"), trVDR("Forward"), trVDR("End"));
