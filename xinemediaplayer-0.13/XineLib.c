...
isyslog("[xinemedia] Playback finished\n");
    ...
    void XineLib::Init() THROWS {
      ...
      int audioHD;
      cSetupLine *lLine = Plugin::GetSetupLine("AudioOverHD","reelbox");
        if(lLine && lLine->Value())
            audioHD=atoi(lLine->Value());
      if(useOld) {
        ...
        audioDriverId = audioHD ? "audio_out_reel" : "alsa";
        aoPort_ = ::xine_open_audio_driver(xine_, audioDriverId, const_cast<VdrXineMpIf *>(vxi));
      } else {
        audioDriverId = audioHD ? "hde-audio" : "alsa";
      ...
      //Set alsa mode
      if (!audioHD){
        lLine = Plugin::GetSetupLine("Ac3","reelbox");
        int Ac3 = 0;
...
#define A52_PASSTHRU  12
        ...
        if(lLine && lLine->Value())
           Ac3 = atoi(lLine->Value());
        if (xine_config_lookup_entry(xine_, "audio.output.speaker_arrangement",&entry) == 1) {
           if (Ac3)
               entry.num_value = A52_PASSTHRU;
           else
               entry.num_value = STEREO;
        xine_config_update_entry(xine_, &entry);
        } else
           printf (" ---  could not find entry-key: audio.output.speaker_arrangement \n");
      }
      ...
      if(!useOld) {
        cSetupLine *lLine = Plugin::GetSetupLine("DelayAudio");
        if(lLine && lLine->Value())
          SetParamAVOffset(-1*atoi(lLine->Value()));
        lLine = Plugin::GetSetupLine("DelaySPU");
        if(lLine && lLine->Value())
