...
//printf("type %d flag %d ppts %lld lppts %lld vpts %lld tpts %lld bpts %lld\n",this->parser->pict_type,buf->decoder_flags,this->parser->pts,this->parser->last_pts,img->vpts,this->pts,buf->pts);
//printf("info0 %d info1 %d parser %d fields %d \n",buf->decoder_info[0],buf->decoder_info[1],parser_pts,this->fields);
if(this->stream_info.format != HDE_PIX_FMT_H264){

			if(parser_pts || (this->parser->pict_type == FF_I_TYPE && this->parser->pts > img->vpts )/* || this->stream_info.format != HDE_PIX_FMT_MPEG2*/ || (buf->decoder_info[0]==1))
				if(pts && this->parser->pts && this->parser->pts != AV_NOPTS_VALUE)
//if(!buf->decoder_flags)
					pts = this->parser->pts & HDE_VALID_PTS;
}else{

			if( buf->pts && !(buf->decoder_info[0] >= 3000 && buf->decoder_info[0] < 4000) && (buf->decoder_flags & BUF_FLAG_FRAMERATE) && (buf->decoder_flags & BUF_FLAG_FRAME_END) ) {
				pts = this->parser->pts & HDE_VALID_PTS;
//printf("------------\n");
			}
			else if(this->parser->pict_type == FF_I_TYPE && (buf->decoder_flags & BUF_FLAG_FRAMERATE) && buf->pts){
				pts = this->parser->pts & HDE_VALID_PTS;
//printf("000000000000\n");
			}
			else if(!(buf->decoder_flags & BUF_FLAG_FRAMERATE) && !(buf->decoder_flags & BUF_FLAG_FRAME_START && buf->decoder_flags & BUF_FLAG_FRAME_END) && /*!(buf->decoder_flags & BUF_FLAG_FRAME_START) && */buf->pts && this->parser->pts && this->parser->pts != AV_NOPTS_VALUE){
				pts =  img->vpts & HDE_VALID_PTS;
//printf("111111111\n");
			}
			else if (!buf->pts && !(buf->decoder_flags & BUF_FLAG_FRAMERATE)&& buf->decoder_flags){
				pts = this->parser->pts & HDE_VALID_PTS;
//printf("222222222222\n");
			}
// if (buf->decoder_flags == 6 && this->parser->pict_type == 1 && this->parser->pts){
//pts =  img->vpts & HDE_VALID_PTS;
//printf("type %d flag %d ppts %lld vpts %lld tpts %lld bpts %lld\n",this->parser->pict_type,buf->decoder_flags,this->parser->pts,img->vpts,this->pts,buf->pts);
//}
//printf("33333333333\n");
			skip_hde=0;
}
...
//				pts = img->vpts; // Needed to set pts or hde will use now and thats too late (stillframe till next i-frame)
			if((this->last_skip > 25) && img->vpts && buf->decoder_flags == 6) {
