...
: valid_(true), playListActive_(false), 
  ...
  mode_(expert), caller_(normal), xineCtrl_ (xine_browse), filter_(cl_unknown), id_(-1), exitDir_(BASE_DIR), startDir_(BASE_DIR), startFile_(""),
...
void cFileBrowserStatus::Merge(cFileBrowserStatus &newstatus)
{
    ...
    std::string defaultStartDir = GetDefStartDir(filter_);
    //printf("--cFileBrowserStatus::Merge, newstatus.startDir_ = %s, startDir_ = %s--\n", newstatus.startDir_.c_str(), startDir_.c_str());
    startDir_ = (newstatus.startDir_ != defaultStartDir && newstatus.startDir_ != BASE_DIR) ?  newstatus.startDir_ : startDir_; 
...
void cFileBrowserStatus::Shutdown(std::string dir, std::string file, /*std::string playlist,*/ std::string playListEntry)
{
    ...
    if(mode_ != restricted)  //do not remember dir when caller = mediaplayer
    {
...
{
    //printf("----cFileBrowserStatus::Startup: caller = %d, playListActive_= %d\n", caller_, playListActive_);
    ...
    {
        SetMode(restricted);
        exitDir_ = BASE_DIR; //GetDefStartDir(filter_);
        if(ifTypes_.size() >= 1)
        {
