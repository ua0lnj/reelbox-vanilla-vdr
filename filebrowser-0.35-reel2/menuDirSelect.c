...
cMenuDirSelect::cMenuDirSelect(std::string title, cFileBrowserStatus *newStatus, cFileBrowserStatus *defStatus, cFileCache *cache, cImageConvert *convert, cThumbConvert *thumbConvert)
...
eOSState cMenuDirSelect::EnterDir()
{
    if(Count())
    {
        ...
        return AddSubMenu(new cMenuFileBrowser(title.c_str(), newStatus_, defStatus_, cache_, convert_, thumbConvert_));
    }
    return osContinue;
}
