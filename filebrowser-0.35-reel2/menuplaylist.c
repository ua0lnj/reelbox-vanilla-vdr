...
eOSState cMenuPlayList::ShowFileWithXine(const cPlayListItem &item, bool asPlaylist, bool passInstance, bool showImmediately)
{
...
{
    cMenuFileItem *fileitem = (cMenuFileItem*)GetCurrentFileItem();
    if(fileitem)
    {
...
eOSState cMenuPlayList::ShowPlayListWithXine()
{
     ...
     {
         cMenuFileItem *firstItem = (cMenuFileItem*)GetFileItem(0);
         if(firstItem)
         {
...
int cMenuPlayList::GetNextPlayableItemCyclic(cMenuFileItem *currentitem, bool all) const
{
    ...
    {
        nextitem = (cMenuFileItem*)GetFileItem(i);
    ...
    {
        nextitem = (cMenuFileItem*)GetFileItem(j);
        if((all && nextitem->IsPlayable()) || nextitem->IsOfSimilarType(*currentitem))
        {
            return j;
        }
