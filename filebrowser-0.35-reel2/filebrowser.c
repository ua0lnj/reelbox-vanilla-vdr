...
{    
    ...
    { 
        ...
        return new cMenuDirSelect(titlestring, instance, &defStatus, cache, convert, thumbConvert);
    }
    else
    {   
	return new cMenuFileBrowser(titlestring, instance, &defStatus, cache, convert, thumbConvert);
    }
}
...
bool cPluginFileBrowser::Service(char const *id, void *data)
{
	...
	std::string name = static_cast<FileBrowserSetPlayListEntries*>(data)->name;
        ...
        { 
            newStatus.SetPlayListEntries(entries);
	    newStatus.GetPlayList()->SetName(name);
	    newStatus.SetStartDir(MUSIC_DIR); 
        }
        return true;
    }
