...
{
public:
    cMenuFileBrowser(std::string title, cFileBrowserStatus *newstatus, cFileBrowserStatus *defstatus, cFileCache *cache, cImageConvert *convert, cThumbConvert *thumbConvert);
    ~cMenuFileBrowser();
    /*override*/ void Shutdown();
    /*override*/  eOSState ProcessKey(eKeys Key); 

    //functions are called from menuitems
    /*override*/ eOSState ShowThumbnail(cPlayListItem item, bool hide, bool currentChanged) const;
    ...
    /*override*/ eOSState ShowFileWithXine(const cPlayListItem &item, bool asPlaylist = false, bool passInstance = true, bool showImmediately = false);
    ...
    eOSState ShowPlayListWithXine(bool passInstance = true, bool showImmediately = false);
    ...
    void ClearPlayList();
    void SavePlayList(std::string path, bool clear = true);
    void CopyPlayListToUsb() const;
    void CopyPlayListToDvd() const;
    bool PlayListIsEmpty() const;
    void CopyPlayListToDirectory(std:: string dir = "") const;
    void AdjustPlaylistToTmpPlaylist();
    void InsertCurrentInTmpPlaylist(bool setcurrent = false);
    void RemoveCurrentFromTmpPlaylist();
    bool IsInPlaylist(cMenuFileItem *item) const;
    bool IsInTmpPlaylist(cMenuFileItem *item) const;
    void AddAllMarkedEntriesToTmpPlaylist();
    void AddAllFlaggedEntriesToTmpPlaylist();
    void AddAllTaggedEntriesToTmpPlaylist();
    void RemoveAllEntriesFromTmpPlaylist();
    void AppendCurrentToPlaylist();
    eOSState LoadAndReplayPlayList(cPlayListItem item);
    std::string GetTitleString() const;
    std::string GetPreTitle() const;
    std::string GetExternalActionString() const;
    std::string GetCurrentDir() const {return currentdir_;}
