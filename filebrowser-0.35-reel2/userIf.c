...
{
    //printf("-------------cMenuBrowserNavigateIf::AnyKey-----------\n");
    ...
    if(!menu->GetBrowserSubMenu())
    {
        ...
        menu->GetFinishedTasks();
#endif
        menu->UpdateCurrent(menu->CurrentChanged());
    }
    return state;
}
...
{
    cMenuDirItem *currentDirItem = (cMenuDirItem*)menu->GetCurrentDirItem();
    cMenuFileItem *currentFileItem = (cMenuFileItem*)menu->GetCurrentFileItem();
...
{
    cMenuFileItem *item = (cMenuFileItem*)menu->GetCurrentFileItem();
...
eOSState cMenuBrowserDeleteRequestIf::LeaveState(cMenuFileBrowser *menu, bool ok)
{
    ...
    if(ok)
    {
        ...
        Skins.Message(mtInfo, tr("Deleting"), 3);
    }
    menu->userIf_->EnterState(menu, this);
    return osContinue;
}
...
{
    Skins.Message(mtInfo, tr("Copying"), 3);
}
...
{
    Skins.Message(mtInfo, tr("Moving"), 3); 
}
...
eOSState cMenuBrowserStandardIf::OkKey(cMenuFileBrowser *menu)
{
    ...
    cMenuDirEntryItem *currentItem = (cMenuDirEntryItem*)menu->GetCurrentDirEntryItem();
...
void cMenuBrowserStandardIf::SetHelpMarkUnmark(cMenuFileBrowser *menu)
{
    ...
    cMenuBrowserItem *item = (cMenuBrowserItem*)menu->GetCurrentItem();
...
{
    //printf("-----cMenuBrowserBrowsePlayListIf2::OkKey---\n");
    eOSState state = osContinue;
    cMenuDirItem *currentDirItem = (cMenuDirItem*)menu->GetCurrentDirItem();
    cMenuFileItem *currentFileItem = (cMenuFileItem*)menu->GetCurrentFileItem();
...
{
    cMenuDirEntryItem *currentDirItem = (cMenuDirEntryItem*)menu->GetCurrentDirItem();
    cMenuFileItem *currentFileItem = (cMenuFileItem*)menu->GetCurrentFileItem();
