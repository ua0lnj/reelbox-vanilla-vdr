...
cFileBrowserStatus *cMenuFileBrowserBase::defStatus = NULL;
...
void cMenuFileBrowserBase::MarkAllFiles(bool onlyXinePlayable)
{
    ...
    {
        item = (cMenuBrowserItem*) GetItem(i);
        SetCurrent(Get(i));
...
bool cMenuFileBrowserBase::AllIsMarked() const
{
    for (int i= 0; i < Count(); i++)
    {
        if(IsMarked((cMenuBrowserItem*)GetItem(i)))
            return false;
    }
    return true;
}
...
bool cMenuFileBrowserBase::AllFilesAreMarked(bool onlyXinePlayable) const
{
    for (int i= 0; i < Count(); i++)
    {
        if(GetItem(i)->IsFile() && !IsMarked((cMenuBrowserItem*)GetItem(i)) &&
	(!onlyXinePlayable || static_cast<cMenuFileItem *>((cMenuBrowserItem*)GetItem(i))->IsXinePlayable()))
            return false;
    }
    return true;
}
...
cMenuDirEntryItem const *cMenuFileBrowserBase::GetSingleMarkedDirEntryItem() const
{
...
cMenuFileItem const *cMenuFileBrowserBase::GetSingleMarkedFileItem() const
{
    if(markedEntries_.size() == 1)
    {
...
void cMenuFileBrowserBase::ClearMarks(bool next)
{
    ...
    {
        cMenuBrowserItem *item = (cMenuBrowserItem*)GetItem(*markedEntries_.begin());
...
void cMenuFileBrowserBase::ReMarkItems()
{
    for (int i = 0; i < Count(); i++)
    {
        cMenuBrowserItem *item = (cMenuBrowserItem*)GetItem(i);
...
{
    cMenuBrowserItem *item = (cMenuBrowserItem*)GetCurrentItem();
    if (item && (item->IsFile() || item->IsDir()))
    {
        if(!IsMarked((cMenuBrowserItem*)GetCurrentItem()))
        {
            //select line
            MarkItem((cMenuBrowserItem*)GetCurrentItem());
        }
        else
        {
            //deselect line
            RemoveMark((cMenuBrowserItem*)GetCurrentItem());
        }
        Display();
     }
}
...
bool cMenuFileBrowserBase::AllIsFlagged() const
{
    for (int i= 0; i < Count(); i++)
    {
        if(IsFlagged((cMenuBrowserItem*)GetItem(i)))
            return false;
    }
    return true;
}
...
bool cMenuFileBrowserBase::AllFilesAreFlagged(bool onlyXinePlayable) const
{
    for (int i= 0; i < Count(); i++)
    {
        if(GetItem(i)->IsFile() && !IsFlagged((cMenuBrowserItem*)GetItem(i)) &&
	(!onlyXinePlayable || static_cast<cMenuFileItem *>((cMenuBrowserItem*)GetItem(i))->IsXinePlayable()))
            return false;
    }
    return true;
...
void cMenuFileBrowserBase::ClearFlags(bool next)
{
    ...
    {
        cMenuBrowserItem *item = (cMenuBrowserItem*)GetItem(*flaggedEntries_.begin());
        /* TB: after renaming the item has changed and is NULL */
        if(item)
        {
...
void cMenuFileBrowserBase::FlagUnflagCurrentItem()
{
    ...
    if (item && (item->IsFile() || item->IsDir()))
    {
        if(!IsFlagged((cMenuBrowserItem*)GetCurrentItem()))
        {
            //select line
            FlagItem((cMenuBrowserItem*)GetCurrentItem());
        }
        else
        {
            //deselect line
            RemoveFlag((cMenuBrowserItem*)GetCurrentItem());
        }
        Display();
     }
}
...
bool cMenuFileBrowserBase::AllIsTagged() const
{
    for (int i= 0; i < Count(); i++)
    {
        if(IsTagged((cMenuBrowserItem*)GetItem(i)))
            return false;
    }
    return true;
}
...
bool cMenuFileBrowserBase::AllFilesAreTagged(bool onlyXinePlayable) const
{
    for (int i= 0; i < Count(); i++)
    {
        if(GetItem(i)->IsFile() && !IsTagged((cMenuBrowserItem*)GetItem(i)) &&
	(!onlyXinePlayable || static_cast<cMenuFileItem *>((cMenuBrowserItem*)GetItem(i))->IsXinePlayable()))
            return false;
    }
    return true;
...
void cMenuFileBrowserBase::ClearTags(bool next)
{
    ...
    {
        cMenuBrowserItem *item = (cMenuBrowserItem*)GetItem(*taggedEntries_.begin());
        /* TB: after renaming the item has changed and is NULL */
        if(item)
        {
...
void cMenuFileBrowserBase::TagUntagCurrentItem()
{
    ...
    if (item && (item->IsFile() || item->IsDir()))
    {
        if(!IsTagged((cMenuBrowserItem*)GetCurrentItem()))
        {
            //select line
            TagItem((cMenuBrowserItem*)GetCurrentItem());
        }
        else
        {
            //deselect line
            RemoveTag((cMenuBrowserItem*)GetCurrentItem());
        }
        Display();
     }
} 
...
eOSState cMenuFileBrowserBase::OpenCurrent(bool asPlaylistItem)
{
    if(GetCurrentItem())
    {
        return ((cMenuBrowserItem*)GetCurrentItem())->Open(this, asPlaylistItem);
    }
    return osContinue;
}
...
eOSState cMenuFileBrowserBase::OpenCurrentDir()
{
    ...
    {
        return ((cMenuBrowserItem*)GetCurrentDirItem())->Open(this);
    }
    return osContinue;
}
...
eOSState cMenuFileBrowserBase::UpdateCurrent(bool currentChanged)
{
    if(GetCurrentItem())
    {
        return ((cMenuBrowserItem*)GetCurrentItem())->Update(this, currentChanged);
    }
    return osContinue;
}
...
eOSState cMenuFileBrowserBase::PlayCurrent()
{
    if(GetCurrentItem())
    {
        return ((cMenuBrowserItem*)GetCurrentItem())->Play(this);
    }
    return osContinue;
}
...
cMenuBrowserItem const *cMenuFileBrowserBase::GetItem(itemId id) const
{
    for (int i = 0; i < Count(); i++)
    {
         cMenuBrowserItem *item = (cMenuBrowserItem*)GetItem(i);
...
cMenuDirEntryItem const *cMenuFileBrowserBase::GetDirEntryItem(itemId id) const
{
    for (int i = 0; i < Count(); i++)
    {
         cMenuDirEntryItem *item = (cMenuDirEntryItem *)GetDirEntryItem(i);
         if(item && item->GetId() == id)
         {
             return item;
         }
...
cMenuFileItem const *cMenuFileBrowserBase::GetFileItem(itemId id) const
{
    for (int i = 0; i < Count(); i++)
    {
         cMenuFileItem *item = (cMenuFileItem*)GetFileItem(i);
         if(item && item->GetId() == id)
         {
             return item;
         }
...
void cMenuFileBrowserBase::BuildActualPlayList(const cPlayListItem &item, cPlayList &playlist, bool all)
{
    ...
    {
        fileitem = (cMenuFileItem*)GetFileItem(startPos);
        if(fileitem)
        {
    ...
    {
        fileitem = (cMenuFileItem*)GetFileItem(i);
        if(fileitem)
        {
