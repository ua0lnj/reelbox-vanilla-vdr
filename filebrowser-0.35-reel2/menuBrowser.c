...
cMenuFileBrowser::cMenuFileBrowser(const std::string title, cFileBrowserStatus *newstatus, cFileBrowserStatus *defstatus,cFileCache *cache, cImageConvert *convert, cThumbConvert *thumbConvert)
...
void cMenuFileBrowser::GetFinishedTasks()
{
    cTask *task;
...
void cMenuFileBrowser::DeleteMarkedOrCurrent()
{
    ...
    {
        cMenuDirEntryItem *item = (cMenuDirEntryItem*)GetCurrentDirEntryItem();
...
bool cMenuFileBrowser::ConvertToUTF8()
{
    ...
    for(std::set<itemId>::iterator i = markedEntries_.begin(); i != markedEntries_.end(); i++) {
        ...
        ((cMenuBrowserItem*)GetItem(*i))->UnMark();
        //printf("----%s----\n", cmd.c_str());
    }
    Display();
...
{
    cMenuFileItem *item = (cMenuFileItem*)GetCurrentFileItem();
    if(item)
    { 
...
{
    //printf("MarkFlagTagItems(), status->GetMode() = %d, playlist_->Size() = %d\n", status->GetMode(), playlist_->Size());    
    if(status->GetMode() != browseplaylist2)
    {
	return;
    }  
