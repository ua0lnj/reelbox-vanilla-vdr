...
{
#if VDRVERSNUM < 20301
    if (cPluginManager::GetPlugin("reelbox") != NULL){
	double aspect = 0;
	int HDres = 0;

	for (cSetupLine *sl = Setup.First();sl;sl = Setup.Next(sl))
	{
        //esyslog ("setup Key %s:\t%s-%s  \n", sl->Name(), sl->Plugin(), sl->Value());
            if(sl->Name() && sl->Plugin() && sl->Value())
            {
                if(!strcasecmp(sl->Plugin(),"reelbox") && !strcasecmp(sl->Name(),"HDResolution"))
                {
                    HDres = atoi(sl->Value());
                }
                else if(!strcasecmp(sl->Plugin(),"reelbox") && !strcasecmp(sl->Name(),"HDDisplayType"))
                {
                    aspect = atoi(sl->Value());
                }
            }
        }

        switch(HDres){
	    case 0: m_nHeight = 1080;
		break;
	    case 1: m_nHeight = 720;
		break;
	    case 2: m_nHeight = 576;
		break;
	    case 3: m_nHeight = 480;
		break;
            default: m_nHeight = (cDevice::PrimaryDevice()->GetVideoSystem() == vsPAL)?576:480;
        }
//        if(aspect == 0)
        {
            m_nWidth = m_nHeight * 4 / 3;
        }
//        else
        {
//            m_nWidth = m_nHeight * 16 / 9;
        }
    ...
    cDevice::PrimaryDevice()->GetVideoSize((int&)m_nWidth, (int&)m_nHeight, (double&)aspect);
...
#endif
esyslog("image m_nHeight %d m_nWidth %d \n",m_nHeight,m_nWidth);
